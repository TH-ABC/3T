/**
 * ==========================================
 * LOGIC_ORDER_SYNC.GS: MAPPING & SYNC V35.0
...
function handleBatchUpdateOrder(data) {
  try {
    ...
    // Chuẩn hoá month từ UI (02/2026 -> 2026-02)
    const month = normalizeMonthYYYYMM(data.month);
    if (!month) {
      return { success: false, error: "Sai month. Cần 'YYYY-MM' hoặc 'MM/YYYY' (vd: 2026-02 hoặc 02/2026)." };
    }

    // Chỉ update đúng sheet Orders theo tháng
    ...
    for (let r = 1; r < v.length; r++) {
      if (ids.has(String(v[r][1]))) list.push(s.getRange(r + 1, col).getA1Notation());
    }

    if (list.length > 0) s.getRangeList(list).setValue(data.value ? "TRUE" : "FALSE");
    return { success: true, updatedCount: list.length, sheet: sheetName };
  } catch (e) {
    return { success: false, error: e.toString() };
  }
}

// 6. Cập nhật trạng thái Designer hàng loạt (FIX nhầm sheet: dùng STRICT month, không sheet[0])
function handleBatchUpdateDesigner(data) {
  const ss = SpreadsheetApp.openById(data.fileId);
  const ids = new Set(data.orderIds.map(String));
  const val = data.value ? "TRUE" : "FALSE";

  const pick = getOrdersSheetStrict(ss, data.month);
  if (!pick.sheet) return { success: false, error: pick.error };

  const s0 = pick.sheet;
  const d0 = s0.getDataRange().getValues();
  const list0 = [];
  for (let r = 1; r < d0.length; r++) {
    if (ids.has(String(d0[r][1]))) list0.push(s0.getRange(r + 1, 14).getA1Notation());
  }
...
// 7. Đồng bộ trạng thái Printway (FIX nhầm sheet: thêm month strict, không sheet[0])
function handleSyncPW(fid, month) {
  try {
...
// 8. Đồng bộ trạng thái Fulfillment (FIX nhầm sheet: thêm month strict, không sheet[0])
function handleSyncFF(fid, month) {
  try {
    const ss = SpreadsheetApp.openById(fid);
...
// 9. Đồng bộ tổng hợp Fulfillment (PW + FF)
// (giữ nguyên logic, chỉ forward thêm month nếu truyền vào)
function handleSyncFulfillment(fid, month) {
  const resPW = handleSyncPW(fid, month);
  const resFF = handleSyncFF(fid, month);
  return { success: resPW.success || resFF.success, updatedCount: (resPW.updatedCount || 0) + (resFF.updatedCount || 0) };
}

// 10. Lấy lịch sử biến động Store (Listing/Sale)
...
// 11. Chốt số liệu hàng ngày tự động (Snapshot)
