...
export const DesignerList: React.FC<DesignerListProps> = ({ user, onProcessStart, onProcessEnd }) => {
  ...
  const getUniqueValues = (key: keyof Order | 'storeName' | 'category' | 'isDesignDone'): string[] => { const values = new Set<string>(); orders.forEach(order => { let val = ''; if (key === 'storeName') val = getStoreName(order.storeId); else if (key === 'category') val = skuMap[order.sku.toLowerCase().trim()] || '(Chưa phân loại)'; else if (key === 'isDesignDone') val = order.isDesignDone ? "Đã xong" : "Chưa xong"; else val = String(order[key as keyof Order] || ''); if (val) values.add(val); }); return Array.from(values).sort(); };
