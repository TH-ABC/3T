  ...
  const loadData = async (monthToFetch: string) => {
    ...
    if (rawOrders.length > 0) {
      ...
      const isDesignerOnlineOrder = (o: Order) => {
          ...
          const handlerRaw = (o.handler || '').toLowerCase().trim();
          
          if (!actionRoleRaw && !handlerRaw) return false;

          // 1. If assigned to the generic role "Designer Online" or "Designer"
          if (actionRoleRaw === 'designer online' || actionRoleRaw === 'designer') return true;
          
          // 2. If assigned to the current user specifically (by Username, Full Name or Email)
          if (actionRoleRaw === currentUsername || handlerRaw === currentUsername) return true;
          if (currentFullName && (actionRoleRaw === currentFullName || handlerRaw === currentFullName)) return true;
          if (currentEmail && (actionRoleRaw === currentEmail || handlerRaw === currentEmail)) return true;
          
          // 3. Check if the assigned user has a designer role
          const assignedUser = (usersData as any).find((u: User) => 
            (u.username || '').toLowerCase().trim() === actionRoleRaw || 
            (u.fullName || '').toLowerCase().trim() === actionRoleRaw ||
            (u.email || '').toLowerCase().trim() === actionRoleRaw
          ); 
          const assignedRole = (assignedUser?.role || '').toLowerCase().trim();
          return assignedRole === 'designer online' || assignedRole === 'designer';
      };
      ...
      if (userRole === 'admin' || userRole === 'ceo' || userRole === 'leader') {
          ...
          if (scope === 'none') {
              ...
              filteredOrders = ordersInMonth.filter(o => {
                const ar = (o.actionRole || '').toLowerCase().trim();
                const hr = (o.handler || '').toLowerCase().trim();
                return ar === currentUsername || hr === currentUsername || 
                       (currentFullName && (ar === currentFullName || hr === currentFullName)) ||
                       (currentEmail && (ar === currentEmail || hr === currentEmail));
              });
          } else {
              // Default for Designer Online role or users with 'all' permission
