...
export const FinanceBoard: React.FC<FinanceBoardProps> = ({ user }) => {
  ...
  const summary = useMemo(() => {
    ...
    return { totalIncomeUsd, totalExpenseUsd, balanceUsd: totalIncomeUsd - totalExpenseUsd, paymentTotalUsd, fundIncomeUsd, fundExpenseUsd, printwayOutUsd, printwayRefundUsd, salaryOutUsd, ebayOutUsd, storeNetFlowUsd: paymentTotalUsd - printwayOutUsd - ebayOutUsd };
  ...
  const showDetailModal = (type: 'income' | 'expense') => {
    if (type === 'income') {
      setCalculationDetail({
        ...
        formula: "Quỹ Công Ty + Funds + PW Refund",
        items: [
          { label: "Nguồn Quỹ Công Ty (USD)", value: formatCurrency(summary.fundIncomeUsd), icon: Wallet },
          { label: "Store Funds (USD)", value: formatCurrency(summary.paymentTotalUsd), icon: Landmark },
      ...
      setCalculationDetail({
        ...
        formula: "Chi Quỹ + PW Payment + Ebay + Lương NV",
        items: [
    ...
    <div className="p-4 sm:p-6 h-full flex flex-col bg-gray-100 gap-6 overflow-x-hidden pb-20 font-sans text-gray-800">
      {canViewSummary && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 animate-fade-in">
            ...
            <div className="bg-white rounded-3xl p-6 shadow-sm border border-slate-200">
                <div className="flex justify-between items-start"><div className="p-3 bg-amber-50 text-amber-600 rounded-2xl"><ArrowRightLeft size={24}/></div><span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Dòng Tiền Store</span></div>
      ...
      <div className="bg-white p-2 rounded-3xl shadow-sm border border-slate-200 flex flex-col md:flex-row items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
            ...
            }} className={`px-6 py-2.5 rounded-xl text-[10px] font-black uppercase text-white shadow-lg ${activeTab === 'printway' || activeTab === 'ebay' ? 'bg-emerald-600' : 'bg-indigo-600'}`}>
              <Plus size={18} /> {activeTab === 'printway' || activeTab === 'ebay' ? 'Tải Excel' : 'Thêm mới'}
            </button>
        </div>
      </div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-md z-[300] flex items-center justify-center p-4">
           <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-lg overflow-hidden animate-slide-in">
              <div className="px-8 py-6 border-b border-slate-100 flex justify-between items-center bg-gray-50/50">
                 ...
                 <button onClick={() => setIsAddModalOpen(false)} className="p-2 text-slate-400 hover:text-slate-900"><X size={24}/></button>
              </div>
              <form onSubmit={handleAddTransaction} className="p-8 space-y-4">
                 <div className="grid grid-cols-2 gap-4">
                    <div>
                       <label className="text-[10px] font-black text-slate-400 uppercase mb-1.5 block">Nhóm thu chi</label>
                       <select value={formData.category} onChange={e => setFormData({...formData, category: e.target.value})} className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-xs font-bold outline-none focus:ring-2 focus:ring-indigo-500/20 transition-all">
                          {meta.categories.map(c => <option key={c} value={c}>{c}</option>)}
                       </select>
                    </div>
                    <div>
                       <label className="text-[10px] font-black text-slate-400 uppercase mb-1.5 block">Phân loại</label>
                       <select value={formData.subCategory} onChange={e => setFormData({...formData, subCategory: e.target.value})} className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-xs font-bold outline-none focus:ring-2 focus:ring-indigo-500/20 transition-all">
                 ...
                 <div>
                    <label className="text-[10px] font-black text-slate-400 uppercase mb-1.5 block">Nội dung diễn giải</label>
                    <input type="text" value={formData.description} onChange={e => setFormData({...formData, description: e.target.value})} className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-xs font-bold outline-none focus:ring-2 focus:ring-indigo-500/20 transition-all" required />
                 </div>
                 <div className="grid grid-cols-3 gap-4">
                    <div>
                       <label className="text-[10px] font-black text-slate-400 uppercase mb-1.5 block">Số lượng</label>
                       <input type="number" value={formData.quantity} onChange={e => setFormData({...formData, quantity: Number(e.target.value)})} className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-xs font-bold outline-none" />
                    </div>
                    <div className="col-span-2">
                       <label className="text-[10px] font-black text-slate-400 uppercase mb-1.5 block">Đơn giá (VNĐ)</label>
                       <input type="number" value={formData.unitPrice} onChange={e => setFormData({...formData, unitPrice: Number(e.target.value)})} className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-xs font-bold outline-none" />
                    </div>
                 </div>
                 <div className="pt-6 flex gap-4">
                    <button type="button" onClick={() => setIsAddModalOpen(false)} className="flex-1 py-4 bg-slate-100 text-slate-500 rounded-2xl font-black text-xs uppercase tracking-widest">Hủy</button>
                    <button type="submit" disabled={isUploading} className="flex-[2] py-4 bg-indigo-600 text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl flex items-center justify-center gap-2">
                       {isUploading ? <Loader2 size={16} className="animate-spin" /> : <Save size={16}/>} Lưu Chi Phí
                    </button>
                 </div>
              </form>
           </div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-md z-[300] flex items-center justify-center p-4">
           <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-md overflow-hidden animate-slide-in border border-white/20">
              <div className="px-8 py-6 border-b border-slate-100 flex justify-between items-center bg-gray-50/50">
                 ...
                 <button onClick={() => { setIsAddPaymentOpen(false); setIsNewStoreMode(false); setIsNewRegionMode(false); }} className="p-2 text-slate-400 hover:text-slate-900 transition-all"><X size={24}/></button>
              </div>
              <form onSubmit={handleAddPayment} className="p-8 space-y-5">
                 {/* Store Section */}
                 <div>
                    <div className="flex justify-between items-center mb-1.5">
                       <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Tên Store</label>
                       <button 
                         type="button" 
                         onClick={() => { setIsNewStoreMode(!isNewStoreMode); setPaymentData({...paymentData, storeName: ''}); }}
                         className={`p-1.5 rounded-lg transition-all ${isNewStoreMode ? 'bg-indigo-600 text-white shadow-md' : 'bg-slate-100 text-slate-400 hover:text-indigo-600'}`}
                    ...
                    {isNewStoreMode ? (
                        <input 
                          ...
                          className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3.5 text-xs font-bold outline-none focus:ring-2 focus:ring-indigo-500/20" 
                        ...
                        <select 
                          ...
                          className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3.5 text-xs font-bold outline-none focus:ring-2 focus:ring-indigo-500/20 cursor-pointer shadow-sm appearance-none" 
                          required
                        >
                           <option value="">-- Chọn Store đã có --</option>
                 ...
                 <div className="grid grid-cols-2 gap-4">
                    <div>
                       <div className="flex justify-between items-center mb-1.5">
                          <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Vùng (Region)</label>
                          <button 
                            type="button" 
                            onClick={() => { setIsNewRegionMode(!isNewRegionMode); setPaymentData({...paymentData, region: 'Us' as any}); }}
                            className={`p-1.5 rounded-lg transition-all ${isNewRegionMode ? 'bg-indigo-600 text-white shadow-md' : 'bg-slate-100 text-slate-400 hover:text-indigo-600'}`}
                            title="Thêm mới Region"
                          >
                             <Plus size={14}/>
                          </button>
                       </div>
                    ...
                    <div>
                       <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-1.5 block">Số tiền gốc</label>
                       <input 
                         type="number" 
                         value={paymentData.amount === 0 ? '' : paymentData.amount} 
                         onChange={e => setPaymentData({...paymentData, amount: Number(e.target.value)})} 
                         className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3.5 text-xs font-bold outline-none focus:ring-2 focus:ring-indigo-500/20 shadow-sm" 
                         placeholder="0"
                         required 
                       />
                    </div>
                 ...
                 <div className="pt-6 flex gap-4">
                    <button type="button" onClick={() => { setIsAddPaymentOpen(false); setIsNewStoreMode(false); setIsNewRegionMode(false); }} className="flex-1 py-4 bg-slate-100 text-slate-500 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-slate-200 transition-colors">Hủy</button>
                    <button type="submit" disabled={isUploading} className="flex-[2] py-4 bg-emerald-600 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-xl shadow-emerald-100 hover:bg-emerald-700 active:scale-95 transition-all flex items-center justify-center gap-2">
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-md z-[300] flex items-center justify-center p-4">
           <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-lg overflow-hidden animate-slide-in">
              ...
              <div className="p-8">
                 ...
                 >
                    <Upload className={`w-12 h-12 ${isDraggingPrintway ? 'text-indigo-600' : 'text-slate-300'}`} />
                    <p className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Kéo thả file Excel Printway vào đây</p>
                 ...
                 {uploadData.length > 0 && (
                   <div className="mt-6 p-4 bg-emerald-50 rounded-2xl border border-emerald-100 flex items-center justify-between">
                      <div className="flex items-center gap-3 text-emerald-700">
                         <CheckCircle size={20}/>
                         <span className="text-xs font-bold">Đã phân tích {uploadData.length} dòng dữ liệu.</span>
                      </div>
                      <button onClick={handleUploadPrintway} disabled={isUploading} className="px-6 py-2 bg-emerald-600 text-white rounded-xl text-[10px] font-black uppercase shadow-md flex items-center gap-2">
           ...
           <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-lg overflow-hidden animate-slide-in">
              ...
              <div className="p-8">
                 <div 
                   ...
                   className={`border-2 border-dashed rounded-[2rem] h-48 flex flex-col items-center justify-center gap-3 transition-all cursor-pointer ${isDraggingEbay ? 'bg-amber-50 border-amber-500' : 'bg-slate-50 border-slate-200 hover:bg-slate-100'}`}
                 >
                    <ShoppingBag className={`w-12 h-12 ${isDraggingEbay ? 'text-amber-600' : 'text-slate-300'}`} />
                    <p className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Kéo thả file Excel Ebay vào đây</p>
                 ...
                 {ebayUploadData.length > 0 && (
                   <div className="mt-6 p-4 bg-emerald-50 rounded-2xl border border-emerald-100 flex items-center justify-between">
                      <div className="flex items-center gap-3 text-emerald-700">
                         <CheckCircle size={20}/>
                         <span className="text-xs font-bold">Đã phân tích {ebayUploadData.length} dòng Ebay.</span>
                      </div>
                      <button onClick={handleUploadEbay} disabled={isUploading} className="px-6 py-2 bg-emerald-600 text-white rounded-xl text-[10px] font-black uppercase shadow-md flex items-center gap-2">
                        {isUploading ? <Loader2 size={14} className="animate-spin" /> : "Lưu vào Sheet"}
                      </button>
                   </div>
                 )}
