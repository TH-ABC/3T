...
const OrderList: React.FC<OrderListProps> = ({ user, onProcessStart, onProcessEnd }) => {
  ...
  const handleSubmitOrder = async (e: React.FormEvent) => {
    ...
    if (isEditMode) {
        // --- EDIT MODE (Chỉ sửa dòng hiện tại) ---
        ...
        // Tạo danh sách các dòng đơn hàng cần thêm (mỗi SKU là 1 dòng)
        ...
        if (needsFileCreation) {
             // === BLOCKING MODE (Tạo file mới) ===
             setIsSubmitting(true);
             if (onProcessStart) onProcessStart();

             try {
                 // 1. Gửi dòng đầu tiên để kích hoạt tạo file
                 await sheetService.addOrder(ordersToCreate[0]);
                 
                 // 2. Nếu có nhiều dòng, gửi tiếp các dòng còn lại (parallel)
            ...
            // === OPTIMISTIC MODE (Thêm nhanh) ===
            // Check trùng ID cục bộ
            ...
            setIsModalOpen(false);
            // Cập nhật UI ngay lập tức với TẤT CẢ các dòng sản phẩm
            ...
            Promise.all(ordersToCreate.map(order => sheetService.addOrder(order)))
                .then(() => {
                     console.log(`Đã đồng bộ ${ordersToCreate.length} dòng cho đơn hàng ${orderIdToAdd}.`);
                })
                .catch((error) => {
